...
#define MAPPING_TYPE 2                                              // トレリスシェーピングのマッピングタイプ
...
void run_mapping () {
    ...
    // 符号化
    inverse_parity_check_coding(s, z, NUM_SUBCARRIER);
...
void run_calc_papr_ccdf () {
    ...
    // 出力ファイルを開く
    fp = fsopen("w", "./Result/prpr_ccdf_%d-QAM_%d-subs(TS).dat", NUM_QAM, NUM_SUBCARRIER, MAPPING_TYPE);

    for (papr = 3.0; papr < 13.0; papr += 0.2) {
...
void run_calc_mean_papr () {
    ...
    // 出力ファイルを開く
    fp = fsopen("w", "./Result/mean_papr_%d-QAM_%d-subs(TS).dat", NUM_QAM, NUM_SUBCARRIER, MAPPING_TYPE);
...
void run_calc_ber () {
    ...
    for (ebn0 = 6; ebn0 < 26; ebn0++) {
        ...
        for (i = 0; i < NUM_OFDM; i++) {
            ...
            // 符号化
            inverse_parity_check_coding(s1, z1, NUM_SUBCARRIER);
...
void run_calc_time () {
    ...
    // 出力ファイルを開く
    fp = fsopen("w", "./Result/time_%d-QAM_%d-subs(lsb_extended).dat", NUM_QAM, NUM_SUBCARRIER, MAPPING_TYPE);
