...
void run_mapping () {
    ...
    // 符号化
    inverse_parity_check_coding(b, z, NUM_B * NUM_SUBCARRIER);
    ...
    // マッピングを出力
    fp = fsopen("w", "raw_mapping_%d-QAM_%d-subcarriers(lsb).dat", NUM_QAM, NUM_SUBCARRIER);
    ...
    // マッピングを出力
    fp = fsopen("w", "caf_mapping_%d-QAM_%d-subcarriers(lsb).dat", NUM_QAM, NUM_SUBCARRIER);
    print_map(fp, t, OVER_SAMPLING_FACTOR * NUM_SUBCARRIER);
    ...
    // マッピングを出力
    fp = fsopen("w", "ts_mapping_%d-QAM_%d-subcarriers(lsb).dat", NUM_QAM, NUM_SUBCARRIER);
    print_map(fp, t, OVER_SAMPLING_FACTOR * NUM_SUBCARRIER);
...
void run_calc_normalized_ccdf () {
    ...
    // 出力ファイルを開く
    fp = fsopen("w", "ccdf_normalized_%d-QAM_%d-subcarriers(lsb).dat", NUM_QAM, NUM_SUBCARRIER);
    ...
    for (power = 2.0; power < 9.5; power += 0.5) {
        ...
        for (i = 0; i < NUM_OFDM; i++) {
            ...
            for (j = 0; j < NUM_SELECTED_MAPPING; j++) {
                ...
                // インタリーバを符号化
                convolutional_coding(x, y, NUM_B * NUM_SUBCARRIER);

                // 信号をインタリーブする
                interleaver(z, y, NUM_Z * NUM_SUBCARRIER);
...
void run_calc_clipping_ratio_characteristic () {
    ...
    // 出力ファイルを開く
    fp = fsopen("w", "clipping_ratio_chara_%d-QAM_%d-subcarriers(lsb).dat", NUM_QAM, NUM_SUBCARRIER);
...
void run_calc_ber () {
    ...
    // 符号化率を計算
    rate = (double)(NUM_D) / (double)(NUM_S + NUM_Z);

    // コンステレーションを初期化
    construct_constellation(NUM_QAM, MAPPING_TYPE);

    // 出力ファイルを開く
    fp = fsopen("w", "ber_%d-QAM_%d-subcarriers(lsb).dat", NUM_QAM, NUM_SUBCARRIER);
    ...
    for (ebn0 = 6; ebn0 < 25; ebn0++) {
        ...
        for (i = 0; i < NUM_OFDM; i++) {
            ...
            // 符号化
            inverse_parity_check_coding(b1, z1, NUM_B * NUM_SUBCARRIER);
            ...
            // 信号をインタリーブする
            interleaver(z1, y, NUM_Z * NUM_SUBCARRIER);
            ...
            // 復号
            parity_check_decoding(b2, z2, NUM_B * NUM_SUBCARRIER);
